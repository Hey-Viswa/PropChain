...
export default function NetworkTelemetry() {
  ...
  return (
    <div className="bg-surface_container_lowest dark:bg-[#131820] rounded-2xl px-6 py-5 shadow-[0_8px_24px_rgba(0,0,0,0.02)] border border-outline_variant/10">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-bold font-display text-on_surface dark:text-[#e8eaf0]">Network Telemetry</h3>
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-on_surface_variant dark:text-[#9ba3b8]">Live Node Stream</span>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-outline_variant/10">
            {rows.map((r, i) => (
              <tr key={i} className="hover:bg-surface_container_low/50 transition-colors">
                ...
                <td className="py-4 px-2 font-medium text-on_surface dark:text-[#e8eaf0]">{r.type}</td>
                <td className="py-4 px-2 font-semibold text-on_surface dark:text-[#e8eaf0]">{r.value}</td>
                <td className="py-4 px-2 text-on_surface_variant dark:text-[#9ba3b8] text-xs">{r.latency}</td>
