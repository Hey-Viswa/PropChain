    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-surface_container_lowest rounded-2xl p-6 border border-outline_variant/10 shadow-[0_8px_24px_rgba(0,0,0,0.02)]">
          ...
          <p className="text-3xl font-bold text-on_surface">{properties.length}</p>
        </div>
        <div className="bg-surface_container_lowest rounded-2xl p-6 border border-outline_variant/10 shadow-[0_8px_24px_rgba(0,0,0,0.02)]">
      ...
      <Suspense fallback={<div className="h-48 bg-surface_container rounded-xl animate-pulse" />}>
        <div className="bg-surface_container_lowest rounded-2xl border border-outline_variant/10 shadow-[0_8px_24px_rgba(0,0,0,0.02)] p-6">
          ...
          {loading ? (
            <div className="h-48 bg-surface_container rounded-xl animate-pulse" />
          ) : properties.length === 0 ? (
            <p className="flex items-center gap-2 text-on_surface_variant py-4">
               <CheckSquare size={20} />
            ...
            <div className="space-y-4">
              {properties.map((p) => (
                <div key={p._id} className="border border-outline_variant/20 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between hover:bg-surface_container_low transition-colors gap-4">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center shrink-0">
                      <ListTodo size={20} />
                    </div>
                    <div>
                      <p className="font-bold text-on_surface">{p.physicalAddress}</p>
                      <p className="text-xs text-on_surface_variant">Token ID: {p.tokenId}</p>
                    </div>
                  </div>
                  ...
                  <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2">
                     {rejectingId === p._id ? (
                        <div className="flex gap-2 w-full">
                          ...
                          <button
                            className="text-xs font-semibold px-4 py-2 rounded-md bg-surface_container text-on_surface whitespace-nowrap"
                       ...
                       <>
                          <button
                            className="text-xs font-semibold px-4 py-2 rounded-md bg-surface_container text-on_surface_variant hover:bg-surface_container_high"
                            onClick={() => setRejectingId(p._id)}
                          >
                            Decline
                          </button>
                          <button
                            className="text-xs font-semibold px-4 py-2 rounded-md bg-primary text-on_primary hover:bg-primary/90"
